...
def apply_fitness_selection(env):
    ...
    # Pierwszy etap: selekcja probabilistyczna
    survives = fitnesses > torch.rand_like(fitnesses)
    
    # Drugi etap: ograniczenie do maksymalnej populacji
    if survives.sum() > env.params['max_population']:
        _, indices = torch.topk(fitnesses[survives], env.params['max_population'])
        mask = torch.zeros_like(survives, dtype=torch.bool)
        mask[torch.where(survives)[0][indices]] = True
        survives = mask
    
    # Aplikuj selekcję
    env.pop.genotypes = env.pop.genotypes[survives]
    env.pop.positions = env.pop.positions[survives]  # Usuń pozycje zmarłych
