...
def reproduce(env):
    ...
    if female_ind.shape[0] == 0 or male_ind.shape[0] == 0:
    ...
    distances, neighbors = torch.tensor(distances.squeeze(1)), torch.tensor(neighbors.squeeze(1))
    ...
    is_reproducing = torch.logical_and(distances < radius, torch.rand_like(distances) < ((1-env.pop.size/env.params['n_organisms']*2)))
